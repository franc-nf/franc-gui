...
{
    // prepare command line arguments and pass to francd
    ...
    qDebug() << "starting francd " + m_monerod;
    ...
    while(true && !m_app_exit) {
        ...
        if(running(nettype)) {
            ...
            if(counter >= 5) {
                qDebug() << "Killing it! ";
#ifdef Q_OS_WIN
                QProcess::execute("taskkill /F /IM francd.exe");
#else
                QProcess::execute("pkill francd");
#endif
            }
...
{ 
    ...
    qDebug() << status;
    // `./francd status` returns BUSY when syncing.
    ...
    : QObject(parent)
{
    ...
    // Platform depetent path to francd
#ifdef Q_OS_WIN
    m_monerod = QApplication::applicationDirPath() + "/francd.exe";
#elif defined(Q_OS_UNIX)
    m_monerod = QApplication::applicationDirPath() + "/francd";
#endif
