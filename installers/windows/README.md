...
[the official download page](https://franc-nf.fr/downloads/).
...
3. The setup script is written to take the GUI wallet files from a subdirectory named `bin`; so create `installers\windows\bin`, get the zip file of the GUI wallet from [here](https://franc-nf.fr/downloads/), unpack it somewhere, and copy all the files and subdirectories in the single subdirectory there (currently named `monero-gui-0.14.0.0`) to this `bin` subdirectory
