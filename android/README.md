...
# Build GUI
        ...
        cd android/docker
        docker build -t franc-gui-android .
        docker create -it --name franc-gui-android franc-gui-android bash

# Get the apk

        docker cp franc-gui-android:/opt/android/franc-gui/build/release/bin/bin/QtApp-debug.apk .
...
- Using ADB (Android debugger bridge) : 
  ...
  The only place where we are allowed to play is `/data/local/tmp`. So : 
        ...
        adb  push /opt/android/franc-gui/build/release/bin/bin/QtApp-debug.apk /data/local/tmp
